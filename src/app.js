(() => {
  ...
  function renderBoard(puzzle) {
    ...
    board.addEventListener("focus", (event) => { if (event.target !== board) return; const first = characterCells(puzzle)[0]; cellMap.get(coord(first.row, first.column))?.focus(); }, true);
  ...
  function handleKeyboard(event, board, puzzle) {
    ...
    const first = characterCells(puzzle)[0]; if (event.key === "Tab" && event.shiftKey && row === first.row && column === first.column) { event.preventDefault(); document.querySelector("#back")?.focus(); return; }
  ...
  function openPuzzle(id) { const puzzle = puzzles.find((item) => item.id === id); if (!puzzle) return renderIndex(); const route = `#/puzzles/${puzzle.id}`; if (location.hash !== route) { location.hash = route; return; } state = readSaved(puzzle) || defaultState(puzzle); if (state.timerStarted && !state.completed) { state.lastTick = Date.now(); clearInterval(timerInterval); timerInterval = setInterval(tickTimer, 250); } render(); }
